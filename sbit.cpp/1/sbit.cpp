/********************************************************************** 
      ...
      04/01/96  [....]    adding support for embedded grayscale bitmap 
...
// 4701 local used without prior initialization
...
{ 
 	...
 	while (usHeight > 0) 
	{
		...
		*pbySrcBitMap = (uint8)(*pbySrcBitMap >> (MAX_BIT_INDEX - usBitDepth - usBitIndex));
		...
		{ 
 			...
 			{ 
				/* 99.9% of the case */ 
 				*pbyDstBitMap = (uint8)(*pbySrcBitMap & usMask);
			} else { 
				*pbyDstBitMap = (uint8)(usMaxLevel -
						(usMaxLevel - *pbyDstBitMap) * (usMaxLevel - *pbySrcBitMap & usMask) / usMaxLevel);
 			}
			*pbySrcBitMap = (uint8)(*pbySrcBitMap >> usBitDepth); 
    ...
    { 
        ...
        for (usLine = 0; usLine < usNewHeight; usLine++)
        {
            while (usErrorTerm >= usNewHeight) 
            {
                ...
                usErrorTerm = (uint16)(usErrorTerm - usNewHeight); 
            }
            ...
            usErrorTerm = (uint16)(usErrorTerm + usOrgHeight);
        } 
			...
			while (usRowCount > 0)
			{
				...
				{ 
					while (usErrorTerm >= usNewWidth)
 					{ 
						sOrgBits--;                     /* skip a bit */ 
 						usErrorTerm = (uint16)(usErrorTerm - usNewWidth);
 					} 
 					...
 					if (sNewBits == 8)                  /* if to data byte is full */
					{
 					...
 					usErrorTerm = (uint16)(usErrorTerm + usOrgWidth); 
				}
