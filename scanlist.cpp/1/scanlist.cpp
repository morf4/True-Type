...
#define MAX_28BITNUM ((1 << (30 - 3)) -1)
#define MAX_ELEMCOUNT ((1<<30)/sizeof(F26Dot6) -1) 
...
) 
{
    ...
    while (lBandWidth > 0)
    {
        ...
        { 
            sHiCross = (int16)(sHiCross + prevHiList->sCross);      /* add in this line's crossings */
    ...
    { 
        while (prevHiList->sScan <= sHiScan)
        {
            sHiCross = (int16)(sHiCross + prevHiList->sCross);      /* add in high line's crossings */
            prevHiList = prevHiList->prevLink;  /* link to next reversal */ 
        }
        while (prevLoList->sScan <= sLoScan) 
        { 
            sLoCross = (int16)(sLoCross + prevLoList->sCross);     /* add in low line's crossings */
...
{ 
    ...
    {
        return (lHScan * (5 * sizeof(int16*)) +     /* for on/off begin/end max */
...
{ 
    ...
    if (!(usScanKind & SK_SMART))       /* if simple dropout */
    { 
        return (lVScan * (5 * sizeof(int16*)) +     /* for on/off begin/end max */
...
/* setup horizontal intersection array for all cases */
    ...
    if((lHInterCount < 0) || (lHInterCount > MAX_28BITNUM))
        return MEM_SUBALLOC_ERR;
...
/* if doing dropout control, setup X intersection array */ 
    ...
    if (!(usScanKind & SK_NODROPOUT))           /* if any kind of dropout */
    {
        ...
        if((lVInterCount < 0) || (lVInterCount > MAX_28BITNUM)) 
            return MEM_SUBALLOC_ERR;
        ...
        {
            if((lElemCount < 0) || (lElemCount > MAX_ELEMCOUNT))
                return MEM_SUBALLOC_ERR;
...
FS_PUBLIC int32 fsc_BeginElement( 
    ...
    AddScanCallback * ppfnAddHorizScan,         /* horiz add scan return */
    AddScanCallback * ppfnAddVertScan           /* vert add scan return */ 
)
{ 
        ...
        uint16 usScanKind )             /* dropout control value */ 
{
    ...
    int32 lXStart;                      /* on transition */
    int32 lXStop;                       /* off transition */
...
/*  now go through the bitmap from top to bottom */ 
    ...
    { 
        ...
        {
            lXStart = *psHorizOn - sXOffset;
            psHorizOn += STATE.sIxSize;
            lXStop = *psHorizOff - sXOffset; 
            psHorizOff += STATE.sIxSize;
 
            if (lXStart < lXStop)                   /* positive run */ 
            {
                lErrCode = fsc_BLTHoriz(lXStart, lXStop - 1, pulRow); 
            }
            else if (lXStart > lXStop)              /* negative run */
            {
                lErrCode = fsc_BLTHoriz(lXStop, lXStart - 1, pulRow); 
            }
            if (lErrCode != NO_ERR) return lErrCode; 
        } 
...
{ 
    ...
    ScanCallback pfnOn;                           /* on callback */
    ScanCallback pfnOff;                          /* off callback */
...
/*  no pixels above or below, now determine bit placement  */ 
    ...
    { 
        usOnTag = (uint16)*(psOn+1);
        sOnPt = (int16)(usOnTag >> SC_CODESHFT);
        pfnOn = GetVCallBack(usOnTag & SC_CODEMASK);
        fxY1 = pfnOn(lXDrop, &STATE.afxXPoints[sOnPt], &STATE.afxYPoints[sOnPt]); 

        usOffTag = (uint16)*(psOff+1); 
        sOffPt = (int16)(usOffTag >> SC_CODESHFT); 
        pfnOff = GetVCallBack(usOffTag & SC_CODEMASK);
