...
int16 CheckInOffset(TTFACC_FILEBUFFERINFO *a,  uint32 b,  size_t c) 
{
...
/* --------------------------------------------------------------------- */ 
__checkReturn __success(return==NO_ERROR)
int16 CheckOutOffset(TTFACC_FILEBUFFERINFO *a, register uint32 b, register size_t c) 
{ 
    if (a->puchBuffer == NULL) /* a prior realloc may have failed */
    { 
...
int16 ReadByte(TTFACC_FILEBUFFERINFO * pInputBufferInfo, uint8 * puchBuffer, uint32 ulOffset) 
{
    ...
    errCode = CheckInOffset(pInputBufferInfo, ulOffset,sizeof(uint8));
    if (errCode != NO_ERROR)
    { 
        return errCode;
    } 
...
ReadWord(TTFACC_FILEBUFFERINFO * pInputBufferInfo, UNALIGNED uint16 * pusBuffer, uint32 ulOffset) 
{
    ...
    errCode = CheckInOffset(pInputBufferInfo, ulOffset,sizeof(uint16));
    if (errCode != NO_ERROR) 
    {
        return errCode;
    }
...
int16 ReadLong(TTFACC_FILEBUFFERINFO * pInputBufferInfo, UNALIGNED uint32 * pulBuffer, uint32 ulOffset)
{
    ...
    errCode = CheckInOffset(pInputBufferInfo, ulOffset,sizeof(uint32));
    if (errCode != NO_ERROR) 
    { 
        return errCode;
    } 
...
/* ---------------------------------------------------------------------- */
__checkReturn __success(return==NO_ERROR) 
int16 ReadBytes(TTFACC_FILEBUFFERINFO * pInputBufferInfo, __out_ecount(Count) uint8 * puchBuffer, uint32 ulOffset, size_t Count) 
{
    int16 errCode; 

    errCode = CheckInOffset(pInputBufferInfo, ulOffset,sizeof(uint8)*Count);
    if (errCode != NO_ERROR)
    { 
        return errCode;
    } 
