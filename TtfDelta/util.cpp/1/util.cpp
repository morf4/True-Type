...
#ifdef _DEBUG
/* ---------------------------------------------------------------------- */ 
void DebugMsg( LPCSTR msg1, LPCSTR msg2, uint16 CONST cLineNumber) 
{
char szErrorBuf[256]; 
HRESULT hr;

	if (cLineNumber == 0)
 		hr = StringCchPrintfA(szErrorBuf, ARRAYSIZE(szErrorBuf), "DEBUG: %s %s.\n", msg1, msg2); 
	else
 		hr = StringCchPrintfA(szErrorBuf, ARRAYSIZE(szErrorBuf), "DEBUG: %s %s. Line: %d\n", msg1, msg2, cLineNumber); 
    Assert(SUCCEEDED(hr)); 
    if (SUCCEEDED(hr))
 		fprintf(stdout, "%s", szErrorBuf); 
}
/* --------------------------------------------------------------------- */
#endif
