...
) 
{
    ...
    *pchGray = (char)(*pchGray + chCount[usOverBits & usShiftMask]);
    pchGray--;                              /* move backwards through both bitmaps! */
    sGrayColumns--; 

    while (sGrayColumns > 0)                /* for each gray column (after 1st) */ 
    { 
        usGoodBits = (uint16)(usGoodBits - pGSP->usOverScale);
        ...
        *pchGray = (char)(*pchGray + chCount[usOverBits & usShiftMask]);  /* accumulate count */ 
        pchGray--;
        sGrayColumns--;
    }
    return NO_ERR; 
